...
const ctxMeta = await page.evaluate(async () => {
  const db = await new Promise((res, rej) => { const r = indexedDB.open('ai-education-reader'); r.onsuccess = () => res(r.result); r.onerror = () => rej(r.error) })
...
// ---- O. progress failure injection: dirty state survives one failed write and retry ----
await openLibrary()
const progressDocButton = page.locator('[data-testid^="doc-open-"]').first()
const progressDocTestId = await progressDocButton.getAttribute('data-testid')
await progressDocButton.click()
await page.locator('[data-testid="document-reader"]').waitFor({ state: 'visible', timeout: 10000 })
await page.locator('[data-testid="reader-page-img"]').waitFor({ state: 'visible', timeout: 30000 })
await page.evaluate(() => { window.__dshFailNextLastReadPageWrite = true })
await page.locator('[data-testid="reader-page-input"]').fill('7')
await page.locator('[data-testid="reader-page-input"]').press('Enter')
await page.locator('[data-testid="reader-progress-error"]').waitFor({ state: 'visible', timeout: 10000 })
assert(await page.locator('[data-testid="reader-progress-error"]').isVisible(), 'O: failed progress write exposes a retryable Reader error')
await page.locator('[data-testid="reader-progress-retry"]').click()
await page.locator('[data-testid="reader-progress-error"]').waitFor({ state: 'hidden', timeout: 10000 })
assert(await page.locator('[data-testid="reader-progress-error"]').count() === 0, 'O: retry clears the progress error after a successful write')
await page.locator('[data-testid="reader-close"]').click()
await openLibrary()
await page.locator(`[data-testid="${progressDocTestId}"]`).click()
await page.locator('[data-testid="document-reader"]').waitFor({ state: 'visible', timeout: 10000 })
await page.locator('[data-testid="reader-page-img"]').waitFor({ state: 'visible', timeout: 30000 })
assert((await inputVal()).trim() === '7', 'O: retried progress survives close and reopen')
await page.locator('[data-testid="reader-close"]').click()
await page.waitForTimeout(300)
