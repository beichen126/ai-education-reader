...
const first = await switchConversationMode({ conversationId: empty.id, modeId: BUILTIN_PROMPT_IDS.conversationSocratic, now: 10, id: () => 't-empty-a' })
...
const second = await switchConversationMode({ conversationId: empty.id, modeId: BUILTIN_PROMPT_IDS.conversationDeepExplanation, now: 11, id: () => 't-empty-b' })
...
const historySwitch = await switchConversationMode({ conversationId: history.id, modeId: BUILTIN_PROMPT_IDS.conversationExamCoaching, now: 21, id: () => 't-history-b' })
...
const noOp = await switchConversationMode({ conversationId: history.id, modeId: BUILTIN_PROMPT_IDS.conversationExamCoaching, now: 22, id: () => 't-no-op' })
...
const branchSwitch = await switchConversationMode({ conversationId: branchConversation.id, branchId: branch.id, modeId: BUILTIN_PROMPT_IDS.conversationSocratic, now: 31, id: () => 't-branch' })
