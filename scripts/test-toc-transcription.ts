...
{
  const r = parseTocJsonl('{"title":"第一章 绪论","pageLabel":"1","visualIndent":0,"numbering":"第一章"}\n\n{"title":"第一节 研究对象","pageLabel":"3","visualIndent":1,"numbering":"第一节"}\n{"title":"一、自然地理学","pageLabel":"5","visualIndent":2,"numbering":"一、"}\n')
...
// --- single malformed line diagnostic (not dropped silently) ---
{
  ...
  assert(r.ok === true, 'malformed line tolerated when others valid')
  if (r.ok) assert(r.rows.length === 2, 'valid lines kept (got ' + r.rows.length + ')')
}
...
{
  const r = parseTocJsonl('{"title":"四、本书内容和结构","pageLabel":"19","tocPage":7}\n{"title":"五、编排","pageLabel":"19","tocPage":7}')
...
{
  const rows = assignLocalRowIds(parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}\n{"title":"B.1","pageLabel":"2"}').ok ? parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}\n{"title":"B.1","pageLabel":"2"}').rows : [])
...
// --- missing id rejected ---
{
  const rows = assignLocalRowIds(parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}').ok ? parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}').rows : [])
  const v = validateTocStructure(rows, [row('r0001',1)])
  assert(v.ok === false, 'missing id rejected')
}
// --- extra id rejected ---
{
  const rows = assignLocalRowIds(parseTocJsonl('{"title":"A","pageLabel":"1"}').ok ? parseTocJsonl('{"title":"A","pageLabel":"1"}').rows : [])
...
// --- level jump rejected ---
{
  const rows = assignLocalRowIds(parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}\n{"title":"C","pageLabel":"3"}').ok ? parseTocJsonl('{"title":"A","pageLabel":"1"}\n{"title":"B","pageLabel":"2"}\n{"title":"C","pageLabel":"3"}').rows : [])
...
// --- cross-page fixture (B13): page A tail + page B head; transcription merge keeps order ---
{
  const pageA = '[{"title":"第二章 自然地理","pageLabel":"20","tocPage":9},{"title":"第一节 地形","pageLabel":"21","tocPage":9}]'
  const pageB = '[{"title":"一、地质构造","pageLabel":"22","tocPage":10},{"title":"二、地貌过程","pageLabel":"23","tocPage":10},{"title":"第二节 气候","pageLabel":"24","tocPage":10}]'
  // These are JSON arrays (legacy); the NEW pipeline uses JSONL. Merge keeps line order.
  const a = assignLocalRowIds(parseTocJsonl(pageA.replace(/\[/g,'').replace(/\]/g,'').replace(/},{/g,'}\n{').replace(/^\n/,'').replace(/\n$/,'')).ok ? parseTocJsonl(pageA.replace(/\[/g,'').replace(/\]/g,'').replace(/},{/g,'}\n{').replace(/^\n/,'').replace(/\n$/,'')).rows : [])
  const b = assignLocalRowIds(parseTocJsonl(pageB.replace(/\[/g,'').replace(/\]/g,'').replace(/},{/g,'}\n{').replace(/^\n/,'').replace(/\n$/,'')).ok ? parseTocJsonl(pageB.replace(/\[/g,'').replace(/\]/g,'').replace(/},{/g,'}\n{').replace(/^\n/,'').replace(/\n$/,'')).rows : [])
  const all = [...a, ...b.map((x, i) => ({ ...x, id: 'r' + String(a.length + i + 1).padStart(4,'0'), rowOrder: a.length + i }))]
  ...
  // This specific fixture needs levels 1,2,3,3,3,3 pattern: 第二章=1, 第一节=2, 一/二=3, 第二节=2
