...
assert(await page.locator('[data-testid="card-viewer"] h1').count() >= 1, 'Markdown headings are really rendered, not escaped')
  const cardTitleStyle = await page.locator('[data-testid="card-title"]').evaluate(element => {
  const style = getComputedStyle(element)
  return { backgroundColor: style.backgroundColor, borderStyle: style.borderStyle }
  })
  assert(cardTitleStyle.backgroundColor === 'rgba(0, 0, 0, 0)' && cardTitleStyle.borderStyle === 'none', 'the card title has no native button box in dark mode')
  assert((await page.locator('[data-testid="card-rating-value"]').innerText()) === '未评分', 'a legacy card starts in the unrated state')
  await page.locator('[data-testid="card-rating-4"]').click()
  await page.waitForFunction(() => document.querySelector('[data-testid="card-rating-value"]')?.textContent?.includes('4 / 5'))
  const ratedCardId = await page.locator('[data-testid="card-viewer"]').getAttribute('data-card-id')
  const ratingStates = await page.locator('[data-testid^="card-rating-"]').evaluateAll(elements => elements
    .filter(element => /^card-rating-\d$/.test(element.getAttribute('data-testid') || ''))
    .map(element => element.getAttribute('data-active')))
  assert(ratingStates.join('|') === 'true|true|true|true|false', 'selecting 4 fills exactly the first four stars')
  assert(await page.locator('[data-testid="card-rating-4"]').getAttribute('aria-pressed') === 'true', 'the selected score is exposed to assistive technology')
  assert((await openAppDb(page, { store: 'studyCards' })).find(card => card.id === ratedCardId)?.rating === 4, 'the rating is persisted to IndexedDB')
  await page.locator('[data-testid="card-rating-clear"]').click()
  await page.waitForFunction(() => document.querySelector('[data-testid="card-rating-value"]')?.textContent?.includes('未评分'))
  assert((await openAppDb(page, { store: 'studyCards' })).find(card => card.id === ratedCardId)?.rating === undefined, 'a rating can be cleared')
  await page.locator('[data-testid="card-rating-4"]').click()
  await page.waitForFunction(() => document.querySelector('[data-testid="card-rating-value"]')?.textContent?.includes('4 / 5'))
...
await page.waitForTimeout(300)
  await page.locator('[data-testid="card-search"]').fill('')
  await page.waitForTimeout(500)
  await page.locator('[data-testid="card-sort"]').selectOption('rating-desc')
  await page.waitForTimeout(300)
  assert(await page.locator('[data-testid="card-item"]').first().getAttribute('data-card-id') === ratedCardId, 'rating sort puts a rated card before unrated cards')
  assert((await page.locator('[data-testid="card-item-rating"]').first().innerText()).includes('4/5'), 'the list shows the saved rating')
  await page.locator('[data-testid="card-sort"]').selectOption('created-asc')
  await page.waitForTimeout(300)
  await page.locator('[data-testid="card-item"]').first().click()
await page.locator('[data-testid="card-viewer"]').waitFor({ state: 'visible', timeout: 10000 })
...
// ---- 8. conversation rename updates auto card titles, but not a title the user edited ----
await page.locator('[data-testid="learning-center-close"]').click()
const sourceSession = page.locator('[data-testid="history-session"]').filter({ hasText: '学习中心测试' })
const sourceSessionRow = sourceSession.locator('..')
await sourceSessionRow.locator('button[title="操作"]').click()
await sourceSessionRow.getByRole('button', { name: '重命名', exact: true }).click()
await page.locator('[data-testid="session-rename-input"]').fill('重命名后的会话')
await page.locator('[data-testid="session-rename-confirm"]').click()
await page.waitForTimeout(600)
...
// ---- 10. reduced motion / mobile: no horizontal overflow ----
