...
if (shouldRun(scenario)) {
  ...
  const visibleLabels = await page.locator('[data-testid="quick-follow-up-send"]').evaluateAll((buttons) => buttons.map((button) => button.textContent?.trim()))
  assert(visibleLabels.join('|') === 'Q1|Q2|Q3|Q4|Q5|Q6|Q7', 'V200-FCR-07 all ordered quick follow-ups are directly available')
  assert(await page.locator('[data-testid="quick-follow-up-more"]').count() === 0, 'V200-FCR-07 no More disclosure adds an extra click')
  await page.setViewportSize({ width: 390, height: 844 })
  const rowCount = await page.locator('[data-testid="quick-follow-up-send"]').evaluateAll((buttons) => new Set(buttons.map((button) => Math.round(button.getBoundingClientRect().top))).size)
  assert(rowCount >= 2, 'V200-FCR-07 overflow wraps onto another row')
  await context.close()
}
