...
const protocolRows = page.locator('[data-testid="prompt-row"]')
await protocolRows.first().waitFor({ state: 'visible', timeout: 10000 })
assert(await protocolRows.count() > 0, 'protocol definitions remain available in their explicit category')
...
assert(await artifactMenu.locator('[data-testid="message-action-branch"]').count() === 1, 'the message menu exposes 从这里分支')
await artifactMenu.locator('[data-testid="message-action-special"]').click()
const specialMenu = page.locator('[data-testid="message-action-special-menu"]')
await specialMenu.waitFor({ state: 'visible', timeout: 5000 })
assert(await specialMenu.locator('button[role="menuitem"]').count() === 3, '开启特殊分支 exposes exactly three entries')
assert(await specialMenu.locator('button[role="menuitem"]').filter({ hasText: '整理成笔记' }).count() === 1 && await specialMenu.locator('button[role="menuitem"]').filter({ hasText: '生成题目' }).count() === 1, 'the special-branch submenu exposes Note and Quiz')
assert(await specialMenu.locator('button[role="menuitem"]').filter({ hasText: '自定义处理' }).count() === 0, 'the special-branch submenu does not expose a legacy custom processing kind')
await specialMenu.locator('[data-testid="message-action-note"]').click()
