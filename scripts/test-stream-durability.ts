...
// --- 1: partial content flushed & durable after a mid-stream error ---
{
  ...
  void sessionsActions.sendUserMessage(id, 'hi', [])
  await waitForConversation('partial stream acceptance', id, (value) => value?.messages.length === 1)
  await s.waitForPull()
...
// --- 2: title rename during slow preflight survives (placeholder merges current state) ---
{
  ...
  fetchMock = async () => new Response(s.stream, { status: 200, headers: { 'Content-Type': 'text/event-stream' } })
  const id = await sessionsActions.newChat()
  const pending = sessionsActions.sendUserMessage(id, 'hello there', [])
  await waitForConversation('title rename acceptance', id, (value) => value?.messages.length === 1)
  await s.waitForPull()
...
// --- 3: deleting the active conversation aborts the stream; never resurrected ---
{
  ...
  const id = await sessionsActions.newChat()
  void sessionsActions.sendUserMessage(id, 'hi', [])
  await waitForConversation('delete stream acceptance', id, (value) => value?.messages.length === 1)
  await s.waitForPull()
