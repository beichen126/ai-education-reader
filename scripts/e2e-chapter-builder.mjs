...
const manualTocBack = page.locator('[data-testid="reader-toc-back"]')
assert(await manualTocBack.isVisible(), 'B: manual TOC exposes 返回阅读')
await manualTocBack.click()
await page.locator('[data-testid="reader-toc"]').waitFor({ state: 'hidden', timeout: 5000 })
assert(await page.locator('[data-testid="document-reader"]').count() === 1, 'B: manual 返回阅读 keeps Reader open')
