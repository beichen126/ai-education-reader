...
assert(await quickBar.locator('[data-testid="quick-follow-up-send"]').count() === 4, 'desktop shows every configured quick follow-up')
assert(await quickBar.getByRole('button', { name: '发送快捷追问：解释' }).count() === 1, 'pinned quick follow-up is visible first')
assert(await quickBar.locator('[data-testid="quick-follow-up-more"]').count() === 0, 'desktop has no folded more entry')
await page.setViewportSize({ width: 390, height: 844 })
const mobileQuickCount = await quickBar.locator('[data-testid="quick-follow-up-send"]').count()
assert(mobileQuickCount === 4, 'mobile keeps every quick follow-up visible across wrapped rows')
assert(await quickBar.locator('[data-testid="quick-follow-up-more"]').count() === 0, 'mobile has no folded more entry')
...
assert((await page.locator('[data-testid="quick-follow-up-send"]').count()) === 4 && initialHits === 0, 'quick catalog hydration makes no model request')
...
assert(await page.locator('[data-testid="quick-follow-up-send"]').count() === 0, 'quick follow-ups hide while the new assistant is streaming')
