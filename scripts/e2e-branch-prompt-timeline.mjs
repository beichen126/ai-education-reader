...
assert(await page.locator('[data-testid="active-conversation-mode"]').innerText() === '默认', 'root route displays static default despite historical mode snapshot')
...
assert(await page.locator('button[aria-label="切换对话模式"]').count() === 0, 'root route has no mode switch control')
...
assert(await page.locator('[data-testid="active-conversation-mode"]').innerText() === '默认', 'branch route displays static default')
assert(await page.locator('button[aria-label="切换对话模式"]').count() === 0, 'branch route has no mode switch control')
