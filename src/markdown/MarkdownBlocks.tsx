...
type Cur = { v: number }
type RenderCtx = { messageId: string; source: string; annotations?: Annotation[]; onMathAction?: (mathId: string, kind: 'inline' | 'block') => void }
...
function sourceAttrs(node: any, kind: 'text' | 'atomic' | 'container') {
  const start = startOf(node)
  const end = endOf(node)
  if (start < 0 || end < start) return {}
  return kind === 'container'
    ? { 'data-source-start': start, 'data-source-end': end, 'data-source-copy-container': '' }
    : { 'data-source-start': start, 'data-source-end': end, 'data-source-copy-leaf': kind }
}

function literalValueRange(node: any, source: string): { start: number; end: number } | null {
  const outerStart = startOf(node)
  const outerEnd = endOf(node)
  const value = typeof node?.value === 'string' ? node.value : ''
  if (outerStart < 0 || outerEnd < outerStart || !value) return null
  const relative = source.slice(outerStart, outerEnd).indexOf(value)
  return relative < 0 ? null : { start: outerStart + relative, end: outerStart + relative + value.length }
}

function literalLeafAttrs(node: any, source: string) {
  const range = literalValueRange(node, source)
  return range
    ? { 'data-source-start': range.start, 'data-source-end': range.end, 'data-source-copy-leaf': 'text' }
    : sourceAttrs(node, 'atomic')
}
...
function inline(node: any, cur: Cur, ctx: RenderCtx): ReactNode {
  switch (node.type) {
    case 'text': {
      ...
      return <span data-canonical-start={st} data-canonical-end={cur.v} {...sourceAttrs(node, 'text')}>{node.value}</span>
    }
    case 'break': {
      const st = cur.v; cur.v += 1
      return <br data-canonical-start={st} data-canonical-end={cur.v} {...sourceAttrs(node, 'atomic')} />
    }
    case 'strong': return <strong {...sourceAttrs(node, 'container')}>{node.children ? node.children.map((c: any) => inline(c, cur, ctx)) : ''}</strong>
    case 'emphasis': return <em {...sourceAttrs(node, 'container')}>{node.children ? node.children.map((c: any) => inline(c, cur, ctx)) : ''}</em>
    case 'link': {
      ...
      if (safe === null) return <span {...sourceAttrs(node, 'container')}>{children}</span>
      return <a href={safe} {...sourceAttrs(node, 'container')} {...(isExternalHref(safe) ? { target: '_blank', rel: 'noopener noreferrer' } : {})}>{children}</a>
    }
    case 'inlineMath': {
      const st = cur.v; cur.v += 1
      const mathId = mathIdOf(ctx.messageId, 'inline', startOf(node), endOf(node))
      const hl = isMathHl(ctx.annotations, mathId)
      return <span data-canonical-start={st} data-canonical-end={cur.v} {...sourceAttrs(node, 'atomic')} data-math data-math-id={mathId} data-math-kind="inline" data-annotatable="false" className={(css.mathInline || '') + (hl ? ' ' + css.studyMathHighlighted : '')} dangerouslySetInnerHTML={{ __html: kathRender(node.value || '', false) }} onClick={(e) => { e.stopPropagation(); if (ctx.onMathAction) ctx.onMathAction(mathId, 'inline') }}></span>
    }
    case 'inlineCode': {
      const st = cur.v; cur.v += (node.value || '').length
      return <code {...sourceAttrs(node, 'container')}><span data-canonical-start={st} data-canonical-end={cur.v} {...literalLeafAttrs(node, ctx.source)}>{node.value}</span></code>
    }
...
function blockEl(node: any, ctx: RenderCtx, onTableAction?: (tableId: string) => void): ReactNode {
  ...
  const cur: Cur = { v: 0 }
  if (t === 'heading') { const level = node.depth; return createElement('h' + Math.min(6, Math.max(1, level)), { 'data-block-id': bid, 'data-block-type': 'heading', ...sourceAttrs(node, 'container') }, inlineList(node.children, cur, ctx)) }
  if (t === 'paragraph') return <p data-block-id={bid} data-block-type='paragraph' {...sourceAttrs(node, 'container')}>{inlineList(node.children, cur, ctx)}</p>
  ...
  if (t === 'listItem') return <li data-block-id={bid} data-block-type='list-item' {...sourceAttrs(node, 'container')}>{innerNodes(node.children || [], cur, ctx)}</li>
  if (t === 'blockquote') return <blockquote data-block-id={bid} data-block-type='blockquote' {...sourceAttrs(node, 'container')}>{innerNodes(node.children || [], cur, ctx)}</blockquote>
  if (t === 'table') {
    ...
    return (<div className={css.tableScroll + (whole ? ' ' + css.studyTableHighlighted : '')} data-table-id={tid} data-block-id={bid} data-block-type='table'>{onTableAction && <button className={css.tableAction} data-table-action={tid} onClick={(e) => { e.stopPropagation(); onTableAction(tid) }}>⋯</button>}<table {...sourceAttrs(node, 'container')}><tbody>{rows.map((row: any, r: number) => (<tr key={r}>{row.children.map((cell: any, c: number) => { const ccur: Cur = { v: 0 }; const hit = whole || coveredRect(r, c); return <td key={c} data-row={r} data-col={c} className={hit ? css.studyCellHighlighted : undefined}>{inlineList(cell.children, ccur, ctx)}</td> })}</tr>))}</tbody></table></div>)
  }
  if (t === 'math') {
    const mathId = mathIdOf(messageId, 'block', startOf(node), endOf(node))
    const hl = isMathHl(annotations, mathId)
    return <div data-block-id={bid} data-block-type='math' {...sourceAttrs(node, 'atomic')} data-math-id={mathId} data-math-kind="block" data-annotatable='false' className={(css.mathBlock || '') + (hl ? ' ' + css.studyMathHighlighted : '')} dangerouslySetInnerHTML={{ __html: kathRender(node.value || '', true) }} onClick={(e) => { e.stopPropagation(); if (ctx.onMathAction) ctx.onMathAction(mathId, 'block') }}></div>
  }
  if (t === 'code') {
    ...
    return <pre data-block-id={bid} data-block-type='code' data-annotatable='true' {...sourceAttrs(node, 'container')}><code><span data-canonical-start={st} data-canonical-end={cur.v} {...literalLeafAttrs(node, ctx.source)}>{value}</span></code></pre>
  }
  return null
}
