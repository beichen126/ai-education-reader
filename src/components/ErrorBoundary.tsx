    ...
    if (this.state.error) {
      return (
        <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:12, padding:24, textAlign:'center' }}>
          ...
          <div style={{ fontSize:12, color:'#888', maxWidth:480, wordBreak:'break-word' }}>{localizedErrorText(String(this.state.error?.message || this.state.error), tx('应用发生了未知错误。', 'The app encountered an unexpected error.'))}</div>
