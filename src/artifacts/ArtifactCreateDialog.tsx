...
export function ArtifactCreateDialog({ sourceLabel, onSubmit, onCancel, busy, initialKind }: Props) {
  ...
  return (<div className={css.dialog} role="dialog" aria-modal="true" aria-label="创建学习成果">
    ...
    <div>
      ...
      <div className={css.kindRow} role="radiogroup" aria-label="模式">
        {TRANSFORMATION_PRESETS.map((p) => (
          <button key={p.kind} type="button" className={css.filterBtn + (kind === p.kind ? ' ' + css.active : '')} role="radio" aria-checked={kind === p.kind} onClick={() => selectKind(p.kind)}>{p.label}</button>
        ))}
      </div>
    ...
    <div>
      ...
      <textarea className={css.promptArea} value={prompt} aria-label="提示词" onChange={(e) => setPrompt(e.target.value)} />
    </div>
