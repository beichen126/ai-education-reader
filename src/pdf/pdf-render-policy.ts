...
export function clampRenderScale(vp1: { width: number; height: number }): number {
  ...
  if (!(maxEdge > 0) || !(vp1.width > 0) || !(vp1.height > 0)) return 1
  const byEdge = MAX_RENDER_EDGE / maxEdge
  const byPixels = Math.sqrt(MAX_RENDER_PIXELS / (vp1.width * vp1.height))
  return Math.max(0.01, Math.min(MAX_SCALE, byEdge, byPixels))
}
