...
- 移除大型 portable ZIP 恢复中人为设置的 1 GB 解压量与 2 GB 压缩文件“安全上限”；恢复前改为读取浏览器为当前站点实时报告的 `quota - usage`，据此判断新暂存数据能否与现有数据共存。
- 浏览器无法提供配额估算时继续流式写入，并由 OPFS 的实际配额约束兜底；写入途中耗尽配额会清理本次暂存文件、保留原数据并显示具体原因。
- 仍拒绝当前解析器无法表示的 ZIP64 文件（ZIP 文件超过 4 GB），并将提示明确为格式兼容性限制，而不是存储“安全范围”。

### Tests

- 新增动态 origin 配额回归测试，验证容量不足时在解压写盘前终止并报告所需空间与当前剩余空间。
...
- 超过 512 MB 的 portable ZIP 可在支持 OPFS 流式写入的现代浏览器中恢复；该版本引入的固定 ZIP / 解压量限制已由 v2.5.2 的浏览器实时配额判断取代。超大 JSON 备份仍会被拒绝，建议使用完整 ZIP。
...
- **本地数据安全中心**：在设置中集中展示浏览器持久化保护、存储配额风险、附件 / 文档 / 学习卡片占用和最近一次成功完整备份；可重新检查，并通过明确的用户操作申请持久化保护。
- **API 配置检查清单**：逐项检查 API Base URL、Key、模型名和图片能力策略；连接检查只请求 `GET /models`，在服务返回模型列表时继续核对模型名，不发送聊天、图片或文档。
- 新增 v2.5.0 领域测试，覆盖 75% / 90% 配额风险、14 天备份新鲜度和 API 配置字段校验。

### Changed

- ZIP / JSON 导入从浏览器原生确认框改为应用内覆盖确认页：明确列出会被替换的数据与 API Key 清除语义，提供“先导出当前数据”，并要求勾选确认后才能继续。
- 导入仍采用经过校验的整包恢复，不做可能破坏会话、分支、附件、PDF 来源和卡片回链的自动冲突合并。
- 完整备份成功后记录本地时间，供安全中心提示备份状态；该记录不包含用户内容。
- package、README、Roadmap、Changelog、Architecture、Privacy、Testing 与 Development 文档统一到 v2.5.0。

### Fixed

- 修正文档中 PDF 最小缩放仍写为 50% 的过期说明，统一为当前 40%–150% 连续缩放能力。
- 修复 README FAQ 重复编号，并新增发布契约检查，防止版本号与说明文档再次漂移。

### Compatibility

- IndexedDB schema 与 Backup V7 数据格式不变；旧 V1–V7 JSON 和 portable ZIP v2 继续可导入。
- API Key 仍不进入备份；导入完成后需重新配置 Key。API 配置检查不会改变会话的推理强度。
