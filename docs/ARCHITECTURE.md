...
本文档描述 v2.5.1 的实际架构。所有数据与处理默认在浏览器本地完成。
...
- **恢复**：文件选择后只读取 ZIP 头并按块解压。清单必须位于首项并先通过结构/引用校验；二进制直接写入新的 OPFS 暂存路径，全部条目、大小和总量核对后才以一次 IndexedDB 事务替换元数据。解析、配额或提交失败会清除暂存文件，旧数据保持可用。portable ZIP 当前限制为压缩文件 2 GB、解压总量 1 GB；普通 JSON 仍需整体解析。
