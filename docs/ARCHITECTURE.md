...
本文档描述 v2.5.2 的实际架构。所有数据与处理默认在浏览器本地完成。
...
- 数据安全中心：只读统计 OPFS 支持状态、各类占用、origin 配额、持久化保护和最近一次成功备份；统计不读取 PDF / 图片正文，不清理、不删除。用户可通过明确操作请求浏览器持久化保护，但它仍不能替代离线备份。
...
- **备份**：portable ZIP v2 + Backup V7 manifest，逐条迭代文档二进制；大备份在支持 File System Access API 时流式写盘，任一文档/附件二进制无法读取则整个备份失败（无静默缺漏）。不含 API Key。
- **恢复**：文件选择后先读取 ZIP 中央目录并按块解压。清单必须位于首项并先通过结构/引用校验；二进制直接写入新的 OPFS 暂存路径，全部条目、大小和总量核对后才以一次 IndexedDB 事务替换元数据。暂存数据需要与现有数据同时存在，因此恢复前通过 `navigator.storage.estimate()` 读取当前 origin 的 `quota - usage`；估算不可用时由 OPFS 实际写入负责配额判定。解析、配额或提交失败会清除暂存文件，旧数据保持可用。没有产品自定义的 1 GB / 2 GB 容量阈值；当前解析器不支持 ZIP64（ZIP 文件超过 4 GB）。普通 JSON 仍需整体解析。
- **恢复**：是经过校验的整包替换，不是对象级合并。界面在读取文件后先展示覆盖范围，允许取消或先导出当前数据，并要求显式勾选确认；恢复完成后整页重载，确保 PDF.js session、object URL 和领域缓存不会继续引用恢复前的数据。
