...
![status](https://img.shields.io/badge/status-v2.0.2-green?style=flat-square)
...
当前发布：**v2.0.2**。这是面向稳定性的收口版本：补齐失败/取消消息的边界、分支拒绝反馈、Reader 章节入口和页面笔记状态，并把关键回归、浏览器 E2E、Pages 部署纳入同一条发布门禁。
