...
当前发布：**v2.0.3**。这是把 Prompt 工作区、Reader 返回链、紧凑章节范围和 PDF 预览真正收进正式发布门禁的版本：旧数据继续可读，新的 PDF 学习链路可以从资料库、Reader、Context、对话一路往返。

<img src="docs/assets/readme/v203-00-app-shell.webp" alt="AI Education Reader 当前主界面，保留完整侧栏与资料入口" width="100%" />
...
- 把回答继续加工成笔记或题目；历史总结、学习指南和自定义成果仍可查看、编辑和导出；
- 通过本地备份把这些学习关系一起带走。
...
| 我想把内容变成什么？ | **学习成果** | 新建面只保留笔记、题目；历史成果继续可读 |
...
<img src="docs/assets/readme/v203-11-prompt-manager.webp" alt="提示词管理：按会话模式、学习成果、快捷追问和系统协议查看与管理学习工作方式" width="100%" />
...
<img src="docs/assets/readme/v203-02-document-library.webp" alt="本地 PDF 资料库，支持阅读、加入对话和更多操作" width="100%" />

Reader 支持目录导航、页码跳转、阅读位置恢复、深色模式和响应式布局。打开目录后可以点击明确的“返回阅读”按钮，也可以按 Escape 返回；读到某一页时，可以从当前页继续创建章节、选择 Context 或打开页面笔记。
...
<img src="docs/assets/readme/v203-04-ai-toc-review.webp" alt="AI 目录检查：逐项确认标题、层级和页码映射" width="100%" />

没有原生目录时，也可以在 Reader 内直接编辑章节树；章节结构保存后即可用于导航和 Context 选择。长目录支持搜索、Shift 范围选择、按当前层级选择，以及批量调整层级；这些选择只是编辑器里的临时状态，不会污染文档数据。

<img src="docs/assets/readme/v203-05-chapter-editor.webp" alt="在 Reader 内搜索并批量编辑 PDF 章节树" width="100%" />
...
文件或章节加入对话前，可以先打开只读 PDF 预览：从选区第一段开始，或回到文档上次阅读页；在预览中翻页和跳目录不会改变选择、阅读进度或产生 attachment。只有明确点击“加入当前对话”才会提交选中的页面。

<img src="docs/assets/readme/v203-03-document-context-picker.webp" alt="从资料库按书签选择范围并加入当前对话" width="100%" />
...
- 可编辑的 Markdown 学习笔记；
- 带答案、解释和来源的结构化测验；
- 历史总结、学习指南和自定义成果仍可打开、编辑和导出；新建面只提供笔记与题目；
- 会话分支，以及主线 / 分支各自独立的草稿。
...
<img src="docs/assets/readme/v203-07-mobile.webp" alt="390px 移动端 PDF Reader，保留返回、加入对话、目录、笔记和关闭入口" width="45%" />

<img src="docs/assets/readme/v203-09-mobile-history-drawer.webp" alt="390px 移动端展开后的历史会话 drawer，收起时仍保留原 rail 入口" width="45%" />

<img src="docs/assets/readme/v203-10-mobile-rail.webp" alt="390px 移动端收起后的原 rail，历史、新会话、图片、文件、全屏和设置仍可直接访问" width="45%" />

界面提供系统、浅色和深色三档外观，使用设计 token 控制颜色和交互状态，避免深色模式把正文变成刺眼的纯白。

<img src="docs/assets/readme/v203-08-dark-mode.webp" alt="深色模式下的 PDF Reader" width="100%" />
...
<img src="docs/assets/readme/v203-06-settings-byok.webp" alt="BYOK 设置和本地数据导出入口" width="100%" />
