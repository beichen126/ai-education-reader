...
![status](https://img.shields.io/badge/status-v2.5.0-green?style=flat-square)
...
在设置中导出“完整备份 ZIP”，安全保存文件；“本地数据安全中心”会显示浏览器清理保护、配额风险和最近一次完整备份。导入不是自动合并：选择 ZIP / JSON 后会先显示覆盖清单，可以取消、先导出当前数据，或勾选确认后继续。继续导入会整包替换当前会话、PDF、附件、笔记、卡片和界面偏好。ZIP 会一起迁移会话、分支、草稿、附件、PDF 原文件、页面笔记、提示词、学习成果、学习卡片及界面偏好；仍兼容旧版 V1–V7 JSON。API Key 不会写入备份，导入时当前 Key 也会被清除，需要重新配置。
...
书签范围的边界可在下拉栏选择 `[)` 或 `[]`：前者表示包含开始页、不包含结束页，即**左闭右开**；后者表示两端都包含，即**左闭右闭**。设置会保存到文档与同层级章节，刷新、关闭再打开仍会保留；没有旧设置的文档安全默认使用 `[)`。预览和按钮直接使用符号表达，不额外堆叠“实际发送”长文案。
...
快捷追问是预先准备好的真实问题，例如“内容太多，请拆成两轮回答”；“继续往下讲”；”我没听懂“等等。点击后它会进入当前会话历史，而不是一个只在界面上短暂出现的提示；主线和分支都可以继续使用。
...
- 在分页模式下逐页阅读，或在设置中切换为连续滚动；底部缩放支持 40%–150%，也支持滑杆和触控板/鼠标滚轮连续调节。
...
- **API Key 不进入 Backup。** Key 保存在当前浏览器本地设置中，既不进入完整备份 ZIP，也不进入 Backup JSON。
- **Backup 可以迁移学习关系。** 当前完整 ZIP 保存文档关系、会话、分支、页面笔记、学习成果、学习卡片、Prompt 配置和二进制原文件；旧 V1–V7 JSON Backup 仍可导入。
- **导入会先明确提示覆盖。** 当前版本不对会话、分支、附件和文档引用做有损的自动合并；导入前可以一键导出当前数据，并且必须勾选确认后才会替换本地数据。
- **数据安全中心用于提前发现风险。** 它只读取元数据，显示浏览器配额、持久化保护和最近备份，不会读取文档正文或上传内容；浏览器保护不能替代离线备份。
