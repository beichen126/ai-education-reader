...
![status](https://img.shields.io/badge/status-v2.4.3-green?style=flat-square)
...
在设置中导出“完整备份 ZIP”，安全保存文件；在目标浏览器选择“导入备份 ZIP / JSON”并等待校验完成。ZIP 会一起迁移会话、分支、草稿、附件、PDF 原文件、页面笔记、提示词、学习成果、学习卡片及界面偏好；仍兼容旧版 V1–V7 JSON。API Key 不会写入备份，也不会从备份恢复，需要在目标浏览器重新配置。
...
在“设置 → PDF 阅读方式”里选择「单页翻页」或「连续上下滚动」。**点击即自动保存**，不需要再点保存按钮；关闭设置、刷新页面或重新打开 PDF 后都会保持你的选择。连续滚动只渲染屏幕附近的页面，10000 页的文档也只保留十来个页面节点；阅读器底部的缩放控件支持缩小到 50%，宽屏下可以同时看到更多内容，点击百分比可恢复 100%。
...
- 在分页模式下逐页阅读，或在设置中切换为连续滚动；底部缩放支持 50%–150%。
